...
func nextNumberOfSeq(seq []int) int {
	diffs := differences(seq)
	if allZeroes(diffs) {
		return seq[len(seq)-1]
	}

	return seq[len(seq)-1] + nextNumberOfSeq(diffs)
}

func GetSumOfNextInSequences(s []string) (int, error) {
	...
	for _, seq := range seqs {
		totalNexts += nextNumberOfSeq(seq)
	}
...
func main() {
	...
	res, err := GetSumOfNextInSequences(lines)
	if err != nil {
		panic(err)
	}
